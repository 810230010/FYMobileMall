                    ...
                    <div class="dropdown profile-element"> <span>
                        ...
                        <a data-toggle="dropdown" class="dropdown-toggle" href="/admin/page/index">
                            <#if currentUser??>
                                ...
                                <#else>
                                    <span class="clear"> <span class="block m-t-xs"> <strong class="font-bold">未登陆</strong></span></span>
                            </#if>
...
<script>
    $('#datatable').DataTable({
        "ajax": {
            'url': '/admin/sell/listUncheckSellItem',
            "data": function(d) {
                var param = {};
            ...
            {"data":"publishTime","width": "10%","title":"发布时间","orderable": true},
            {
                ...
                "render": function (data, type, row) {
                        return [
                            ...
                            '</a>',
                            '<a class="btn btn-success btn-xs table-action btn-block pass" href="javascript:void(0)">',
                            '审核通过 <i class="fa fa-eye"></i>',
                            '</a>',
                            '<a class="btn btn-danger btn-xs table-action btn-block nopass" href="javascript:void(0)">',
                            '不通过 <i class="fa fa-eye"></i>',
                            '</a>'
                        ].join('');
                }
            },
    ...
    table.on( 'click', '.pass', function () {
        ...
        var data = table.row(tr).data();
        $.ajax({
            url: "/admin/sell/passUserSellItemApply",
            data: {
                sellId: data.sellId
            },
            success: function (result) {
                if(result.code == 0){
                    swal("成功！", "该用户闲置申请已通过！","success");
                    setTimeout(function () {
    ...
    //审核不通过
    table.on( 'click', '.nopass', function () {
        var tr = $(this).closest('tr');
        var data = table.row(tr).data();
        $.ajax({
            url: "/admin/sell/rejectUserSellItemApply",
            data: {
                sellId: data.sellId
            },
            success: function (result) {
                if(result.code == 0){
                    swal("成功！", "该用户申请已拒绝！","success");
                    setTimeout(function () {
                        window.location.reload();
                    }, 1500)
                }
